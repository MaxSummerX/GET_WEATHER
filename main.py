...
def get_weather(location: str='Чебоксары') -> None:
    """
    Получение данных о погоде для указанного города в формате
    JSON и выводит информацию в терминал

    :param location: Название города(по умолчанию: Чебоксары)
    """
...
def save_weather_image(location: str, filename: Optional[str] = None) -> None:
    ...
    :param filename: Имя выходного файла (если оно не задано - по умолчанию <город>.png)
    """
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default='Чебоксары',
        help="Город для которого нужно узнать погоду в PNG"
    )
    parser.add_argument(
        "--image",
        action="store_true",
    ...
    parser.add_argument(
        "--filename",
        type=str,
        help="Имя файла для сохранения картинки (по умолчанию <город>.png)"
    )
